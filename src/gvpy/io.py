...
def read_sadcp(ncfile):
    ...
    sadcp = sadcp.rename_dims({"depth_cell": "depth"})
    sadcp.coords["depth"] = (["depth"], mdepth.data)
    # Fix some attributes
    sadcp.depth.attrs = dict(long_name="depth", units="m")
    sadcp.u.attrs = dict(long_name="u", units="m/s")
    sadcp.v.attrs = dict(long_name="v", units="m/s")
    sadcp.time.attrs = dict(long_name="time", units="")
    # Transpose (re-order dimensions)
    # sadcp = sadcp.transpose("depth", "time")
    # Removing transposing as there may be more dimensions
    # This may work:
    # sadcp = sadcp.transpose("depth", "time", ...)
